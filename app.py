...
flaskkey_path = os.getenv("FLASKKEY_PATH", "flaskkey")
flaskkey_dir = os.path.dirname(flaskkey_path)
if flaskkey_dir:
	os.makedirs(flaskkey_dir, exist_ok=True)
if not os.path.exists(flaskkey_path):
	print("Creating new Flask secret key")
	#create a new key
	with open(flaskkey_path, "w") as f:
		f.write(''.join(random.choice(string.ascii_letters + string.digits) for i in range(50)))
app.secret_key = open(flaskkey_path, "r").read()
...
class Base(DeclarativeBase):
  pass
sqlite_path = os.getenv("SQLITE_PATH", "cardwarskingdom.db")
...
server_init_lock = threading.Lock()
...
def ensure_admin_account():
	...
	admin_username = re.sub(r'[^a-zA-Z0-9]', '', admin_username)
	if admin_username == "":
		admin_username = "admin"

	admin_password = os.getenv("ADMIN_PASSWORD")
	admin_rank_raw = os.getenv("ADMIN_RANK", "0")
	admin_password_reset = parse_bool_env(os.getenv("ADMIN_PASSWORD_RESET"))

	try:
		admin_rank = int(admin_rank_raw)
	except ValueError:
		admin_rank = 0

	first_admin = Admin.query.first()
	target_admin = Admin.query.filter_by(username=admin_username).first()

	#if a password was supplied, allow creating a new admin directly from environment variables
	if admin_password and target_admin is None:
		new_admin = Admin(
		...
		db.session.commit()
		Log("server", f"Created admin account from environment variables: {admin_username}")
		return

	#if reset is explicitly enabled, rotate password for target admin
	if admin_password and target_admin is not None and admin_password_reset:
		target_admin.password = bcrypt.generate_password_hash(admin_password).decode('utf-8')
		target_admin.rank = admin_rank
		db.session.commit()
		Log("server", f"Reset password for admin account from environment variables: {admin_username}")
		return

	#fall back to autogenerated admin on first startup when no admin exists
	if not first_admin:
...
def initialize_server():
	...
	with server_init_lock:
		...
		Log("server", "Initializing server state")
...
@app.before_request
def initialize_server_before_request():
	initialize_server()
...
def SystemBan(username):
	if DEV_MODE and IsDevModeUser(username):
		return
...
@login_required
@app.route("/admin/misc", methods=['GET', 'POST'])
def AdminMisc():
	if not isAdmin(current_user):
		return abort(404)
	...
	if request.method == 'POST':
		...
		existing = DevModeUser.query.filter_by(username=user_id).first()
		if existing is None:
			db.session.add(DevModeUser(username=user_id, author=current_user.username, time=int(time.time())))
			db.session.commit()
			Log("admin", f"{current_user.username} enabled DEV mode for user: {user_id}")
		return redirect("/admin/misc")

	users = DevModeUser.query.order_by(DevModeUser.time.desc()).all()
	for user in users:
		if user.time is not None:
			user.time = datetime.fromtimestamp(user.time, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')
	return render_template('admin_misc.html', dev_mode=DEV_MODE, dev_users=users)

@login_required
@app.route("/admin/misc/devmode/remove/<string:user_id>")
def AdminMiscDevModeRemove(user_id):
	if not isAdmin(current_user):
		return abort(404)
	entry = DevModeUser.query.filter_by(username=user_id).first()
	if entry is not None:
		db.session.delete(entry)
		db.session.commit()
		Log("admin", f"{current_user.username} disabled DEV mode for user: {user_id}")
	return redirect("/admin/misc")

@login_required
@app.route("/admin/misc/devmode/global/<string:action>")
def AdminMiscDevModeGlobal(action):
	if not isAdmin(current_user):
		return abort(404)
	global DEV_MODE
	if action == "enable":
		DEV_MODE = True
		Log("admin", f"{current_user.username} enabled global DEV mode")
	elif action == "disable":
		DEV_MODE = False
		Log("admin", f"{current_user.username} disabled global DEV mode")
	else:
		return make_response("Invalid action", 400)
	return redirect("/admin/misc")
...
def GetPlayerWins(player_id):
	...
	#check if user is PVP banned
	if not IsDevModeUser(player_id) and game["Zxcvbnm"]:
		return None
